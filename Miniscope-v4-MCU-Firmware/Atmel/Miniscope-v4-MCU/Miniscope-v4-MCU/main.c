...
// pastI2CWord acts as the "write reg" and received_data act as the value being written
	switch (pastI2CWord) {
		case (NO_WORD):
			...
			break;			
		case (I2C_PYTHON_PASSTHROUGH):		//按位拆包，得到reg地址和value数据
...
void I2C_requested()
{
	I2C_transmitByte(i2c_return_data);		//这是Slave的应答信号
}

void setup_I2C()
{
	// set received/requested callbacks
	I2C_setCallbacks(I2C_received, I2C_requested);  //这个函数是先receive后ack
